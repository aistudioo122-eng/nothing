...
export default function ProductList() {
  ...
  useEffect(() => {
    ...
    const unsubCats = onSnapshot(qCats, (snapshot) => {
      ...
      const f = ['All', ...cats.filter(c => c.type === 'flavor' || c.type === 'category').map(c => c.name)];
      const e = ['All', ...cats.filter(c => c.type === 'occasion' || c.type === 'event' || c.type === 'special').map(c => c.name)];
      setFlavors(f);
      setEvents(e);
    });
  ...
  useEffect(() => {
    ...
    if (activeFlavor !== 'All') {
      result = result.filter(p => {
        const prod = p as any;
        const flavor = prod.flavor || prod.category;
        if (Array.isArray(flavor)) {
          return flavor.includes(activeFlavor);
        }
        return flavor === activeFlavor;
      });
    }
    if (activeEvent !== 'All') {
      result = result.filter(p => {
        const prod = p as any;
        const event = prod.event || prod.occasion || prod.category;
        if (Array.isArray(event)) {
          return event.includes(activeEvent);
        }
